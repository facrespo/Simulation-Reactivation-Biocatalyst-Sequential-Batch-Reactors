...
km=np.random.normal(97,10,naleatorios);
kcat=np.random.uniform(0.7,1.3,naleatorios);
kd=np.random.uniform(0.0005,2.5,naleatorios);
e0=np.random.normal(61,10,naleatorios);
Si=np.random.normal(185,20,naleatorios);
Xopt=0.9;
beta=np.random.uniform(0.9,1,naleatorios);
kr=np.random.uniform(0.1,0.37,naleatorios);
V0=np.random.uniform(0.9,1.1,naleatorios);
Mcat=np.random.uniform(0.9,1.1,naleatorios);
dbeta=0.1;
...
for i in range(0,naleatorios):
    e0_st, Gamma, e_ref_actividad, t_reaccion, t_reactivacion, product, productivity_specif, Productividad_especifica_global  = reactorbioquim.reaccionactivada(km[i], kcat[i], kd[i], e0[i], Si[i], Xopt, nlotes, beta[i], kr[i], V0[i], Mcat[i], dbeta);
    ...
    fich.write("\n");  
    fich.close();
    e0_st, t_reaccion, product, productivity_specif, Productividad_especifica_global = reactorbioquim.reaccion(km[i], kcat[i], kd[i], e0[i], Si[i], Xopt, nlotes, V0[i], Mcat[i]);
