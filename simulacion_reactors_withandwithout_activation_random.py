...
Xopt=0.9;
beta=np.random.normal(0.95,0.85,naleatorios);
...
fich=open("resultadosimulacionactivated_n"  +str(nlotes) + ".txt","w");
...
fich2=open("resultadosimulacionwithoutactivate_n"  +str(nlotes) + ".txt","w");
...
for i in range(0,naleatorios):
    e0_st, betaf, Gamma, e_ref_actividad, t_reaccion, t_reactivacion, product, productivity_specif, Productividad_especifica_global  = reactorbioquim.reaccionactivada(km[i], kcat[i], kd[i], e0[i], Si[i], Xopt, nlotes, beta[i], kr[i], V0[i], Mcat[i], dbeta, xlimit, elimit);
    fich=open("resultadosimulacionactivated_n"  +str(nlotes) + ".txt","a");
    ...
    fich2=open("resultadosimulacionwithoutactivate_n"  +str(nlotes) + ".txt","a");
