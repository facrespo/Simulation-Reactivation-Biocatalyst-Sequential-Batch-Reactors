...
for i in range(0,naleatorios):
    for xo in Xopt:
       e0_st, Gamma, e_ref_actividad, t_reaccion, t_reactivacion, product, productivity_specif, Productividad_especifica_global  = reactorbioquim.reaccionactivada(km[i], kcat[i], kd[i], e0[i], Si[i], xo, nlotes, beta[i], kr[i], V0[i], Mcat[i], dbeta, xlimit, elimit);
       fich=open("resultadosimulacionactivada_n"  +str(nlotes) + ".txt","a");
       fich.write(str(j) + "," + str(km[i]) + "," + str(kcat[i]) + "," + str(kd[i]) + "," + str(e0[i]) + "," + str(Si[i]) + "," + str(xo) + "," + str(nlotes) + "," + str(beta[i]) + ","  + str(kr[i]) + "," + str(V0[i]) + "," + str(Mcat[i]) + ",");
       for p in range(0,nlotes+1):
       ...
       fich.write("\n");  
       fich.close();
       e0_st, t_reaccion, product, productivity_specif, Productividad_especifica_global = reactorbioquim.reaccion(km[i], kcat[i], kd[i], e0[i], Si[i], xo, nlotes, V0[i], Mcat[i], elimit);
       fich2=open("resultadosimulacionsin_n"  +str(nlotes) + ".txt","a");
       fich2.write(str(j) + "," + str(km[i]) + "," + str(kcat[i]) + "," + str(kd[i]) + "," + str(e0[i]) + "," + str(Si[i]) + "," + str(xo) + "," + str(nlotes) + "," +  str(V0[i]) + "," + str(Mcat[i]) + "," );
       for p in range(0,nlotes+1):
